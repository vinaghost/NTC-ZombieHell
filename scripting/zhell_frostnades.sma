...
new isChilled[33], isFrozen[33], frostKilled, novaDisplay[33], Float:glowColor[33][3], Float:oldGravity[33], oldRenderFx[33],
		Float:oldRenderColor[33][3], oldRenderMode[33], Float:oldRenderAmt[33], hasFrostNade[33], nadesBought[33];

public plugin_init()
{
	register_plugin("Zombie Hell: FrostNades",VERSION,"Avalanche");
...
public plugin_log()
{
	...
	if(get_pcvar_num(pcv_enabled) && read_logargc() >= 5)
	{
		...
		if(equal(arg, "killed"))
		{
			...
			if(killedId && Get_BitVar(frostKilled,killedId))
			{
...
public msg_deathmsg(msg_id,msg_dest,msg_entity)
{
	...
	if(!is_user_connected(victim) || !Get_BitVar(frostKilled,victim)) return PLUGIN_CONTINUE;
...
set_user_chillfreeze_speed(id)
{
	if(isFrozen[id]) {
		...
		cs_set_player_maxspeed_auto(id, 0.0);
	}
	else if(isChilled[id]) {
		cs_reset_player_maxspeed(id);
		cs_set_player_maxspeed_auto(id, get_default_maxspeed(id)*(get_pcvar_float(pcv_chill_speed)/100.0));
	}
	else {
		cs_reset_player_maxspeed(id);
		cs_set_player_maxspeed_auto(id, get_default_maxspeed(id));
	}
...
{
	if( id == zhell_get_boss() ) {
		return zhell_get_boss_speed();
	}
	return zhell_get_zombie_speed();
}
